-- UNI-TEL Custom Exam Types Migration - FINAL CORRECTED VERSION
-- This allows users to create custom exam types with custom weightages
-- This version properly handles existing constraints
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'check_weightage_range' 
...
-- 4. Remove the existing restrictive exam_type constraint
ALTER TABLE public.marks_records 
DROP CONSTRAINT IF EXISTS check_exam_type_valid;

-- 5. Add custom exam type validation (allows any text with reasonable length)
ALTER TABLE public.marks_records 
...
-- 13. Update existing records to have default weightage of 100%
